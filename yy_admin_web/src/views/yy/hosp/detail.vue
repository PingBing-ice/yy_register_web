<template>
  <div v-if="hospital!=null" class="app-container">
    <h4>基本信息</h4>
    <table class="table table-striped table-condenseda table-bordered" style="width: 100%">
      <tbody>
        <tr>
          <th style="width: 15%">医院名称</th>
          <td style="width: 35%"><b style="font-size: 14px">{{ hospital.hosname }}</b> | {{ hospital.param.hostypeString }}</td>
          <th style="width: 15%">医院logo</th>
          <td style="width: 35%">
...
export default {
  ...
  methods: {
    ...
    // 返回医院列表
    back() {
      this.$router.push({ path: '/yy/host/list' })
    }
  }
}
</script>
