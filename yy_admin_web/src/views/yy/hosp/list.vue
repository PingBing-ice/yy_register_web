  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row>
      ...
      <el-table-column label="医院logo">
        <template slot-scope="scope">
          <img :src="'data:image/jpeg;base64,'+scope.row.logoData" width="80" alt="图片">
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="230" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="primary" size="mini" @click="schedulePush(scope.row.hoscode)">排班</el-button>
    ...
    <el-pagination
      ...
      :total="total"
      :page-size="limit"
      :page-sizes="[5, 10, 20, 30, 40, 50, 100,total]"
...
export default {
  ...
  created() {
    ...
    hospApi.findByDictCode('Province').then(response => {
      this.provinceList = response.data.dictList
    })
  },
  methods: {
    // 医院列表
    fetchData(page = 1) {
      ...
      hospApi.getPageList(this.page, this.limit, this.searchObj)
        .then(response => {
          ...
          // 总记录数
          this.total = response.data.pages.totalPages
    ...
    updateStatus(id, status) {
      hospApi.updateStatus(id, status, this.page)
        .then(response => {
    ...
    schedulePush(hoscode) {
      this.$router.push({
        path: '/yy/host/schedule',
        query: {
          hoscode
        }
      })
