                ...
                <div class="wrapper">
                  ...
                  <div class="bottom-container">
                    <div class="icon-wrapper">
                      <span class="iconfont"></span>{{ item.hosTypeString }}
                    </div>
                    <div class="icon-wrapper">
                      <span class="iconfont"></span>每天{{ item.releaseTime }}放号
                    </div>
                  </div>
                </div>
...
import hospApi from '@/api/yy/hospital.js'
import dictApi from '@/api/yy/dict.js'

export default {
  //服务端渲染异步，显示医院列表
  asyncData({params, error}) {
    //调用
    // return hospApi.getPageList(1,500,null)
    return hospApi.getList(1, 100, null)
      .then(response => {
        return {
  ...
  methods: {
    ...
    init() {
      ...
      dictApi.findByDictCode('Hostype')
        .then(response => {
          ...
          for (let i = 0; i < response.data.dictList.length; i++) {
            this.hostypeList.push(response.data.dictList[i])
          }
        })
      ...
      dictApi.findByDictCode('Beijin')
        .then(response => {
          ...
          for (let i in response.data.dictList) {
            this.districtList.push(response.data.dictList[i])
          }
        })
    },
    ...
    getList() {
      ...
      //   })
      hospApi.getList(this.page, this.limit, this.hostype, this.districtCode)
        .then(response => {
